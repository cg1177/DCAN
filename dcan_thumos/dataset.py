...
class MyDataset(data.Dataset):  # thumos14
    def __init__(self, opt, subset="train", mode="train", vis=False):
        self.temporal_scale = opt.temporal_scale  # 128
        self.temporal_gap = 1. / self.temporal_scale  # 1/128
        self.subset = subset
        self.mode = mode
        self.feature_path = opt.feature_path
        self.video_info_path = opt.video_info_path
    ...
    def __getitem__(self, index):
        ...
        if self.vis:
            gt_iou_map, match_score_start, match_score_end, match_score_start_short, match_score_end_short = self._get_train_label(
                index)
            return index, video_data, gt_iou_map, match_score_start, match_score_end, match_score_start_short, match_score_end_short

        if self.mode == "train":
            gt_iou_map, match_score_start, match_score_end, match_score_start_short, match_score_end_short = self._get_train_label(
                index)
            return video_data, gt_iou_map, match_score_start, match_score_end, match_score_start_short, match_score_end_short
        else:
    ...
    def _get_train_label(self, index):
        ...
        # gt_len_small = 0.1 * gt_lens
        # gt_len_small = np.maximum(0.1 * gt_lens, 1 * self.skip_videoframes)
        ...
        match_score_start_short = match_score_start
        match_score_end_short = match_score_end

        return gt_iou_map, match_score_start, match_score_end, match_score_start_short, match_score_end_short
...
if __name__ == '__main__':
    from opt_thumos import MyConfig

    arg = MyConfig()
    arg.parse()
    train_loader = torch.utils.data.DataLoader(VideoDataSet(arg, subset="train"),
