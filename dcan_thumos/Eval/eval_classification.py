    ...
    def evaluate(self):
        ...
        if self.verbose:
            print('[RESULTS] Performance on ActivityNet untrimmed video '
                  'classification task.')
            print
            '\tMean Average Precision: {}'.format(ap.mean())
            print
            '\tHit@{}: {}'.format(self.top_k, hit_at_k)
            print
            '\tAvg Hit@{}: {}'.format(self.top_k, avg_hit_at_k)
        self.ap = ap
...
if __name__ == '__main__':
    anet_cls_eval = ANETclassification(ground_truth_filename="./data/activity_net.v1-3.min.json",
                                       prediction_filename="../cls_res/cuhk_val_simp_share.json", verbose=True,
                                       check_status=False)
    anet_cls_eval.evaluate()
