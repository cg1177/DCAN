...
class ProposalGenerator(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.max_len = cfg.max_duration
        self.boundary_sparse = cfg.sparse_sample
        self.len_sparse = cfg.sparse_sample
    ...
    def _get_sample_mask(self):
        ...
        for proposal_len in range(0, self.max_len, self.len_sparse):
            mask_ = []
            for start_index in range(0, self.tscale, self.boundary_sparse):
                end_index = start_index + proposal_len
                ...
                # else:
                #     mask_.append(np.zeros([self.temp_scale, self.num_sample]))
            ...
            mask_ = np.stack(mask_, axis=2)  # tscale,sample_num,start_index,
            mask.append(mask_)
        ...
        mask = np.stack(mask, axis=3).astype(np.float32)  # tscale,sample_num,start_index,p_len
    ...
    def _get_sample_mask_per_proposal(self, start_proposal, end_proposal, tscale, num_sample, num_sample_perbin):
        ...
        mask = np.stack(mask, axis=1)  # tscale,sample_num
        return mask

    def _boundary_matching_layer(self, bm_feature_map):
        feature_size = bm_feature_map.size()  # bm_feature_map.shape: [3][128][100]
        output = torch.matmul(bm_feature_map,
                              self.sample_mask).reshape(feature_size[0], feature_size[1], self.num_sample,
                                                        self.tscale // self.boundary_sparse,
                                                        self.max_len // self.len_sparse).transpose(-1, -2).contiguous()
        return output
